pipeline {
  ...
  environment {
        GIT_NAME = "eea.graylogger"
        SONARQUBE_TAGS = "www.eea.europa.eu,climate-advisory-board.europa.eu,demo-www.eea.europa.eu,forest.eea.europa.eu,www.eea.europa.eu-en,biodiversity.europa.eu,insitu.copernicus.eu,clmsdemo.devel6cph.eea.europa.eu,land.copernicus.eu,demo-ied.eea.europa.eu,www.ied.eea.europa.eu-en,demo-water.devel5cph.eea.europa.eu-freshwater,water.europa.eu-freshwater,demo-wise.02pre.eea.europa.eu-marine,water.europa.eu-marine,water.europa.eu-msfd"
    }
    ...
    stage('Tests') {
      steps {
        parallel(
          ...
          "Python3": {
            node(label: 'docker') {
              sh '''docker pull eeacms/plone-test:5-python3; docker run -i --rm --name="$BUILD_TAG-python3" -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="$GIT_NAME" -e DEVELOP="src/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plone-test:5-python3 -v -vv -s $GIT_NAME'''
            }
          }
        )
      }
        ...
        node(label: 'swarm') {
          script{
            ...
            withSonarQubeEnv('Sonarqube') {
                ...
                sh "export PATH=$PATH:${scannerHome}/bin:${nodeJS}/bin; sonar-scanner -Dsonar.python.xunit.skipDetails=true -Dsonar.python.xunit.reportPath=xunit-reports/*.xml -Dsonar.python.coverage.reportPaths=coverage.xml -Dsonar.sources=./eea  -Dsonar.projectKey=$GIT_NAME -Dsonar.projectName=$GIT_NAME -Dsonar.branch.name=$BRANCH_NAME"
